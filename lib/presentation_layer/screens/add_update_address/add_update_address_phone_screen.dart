...
class AddUpdateAddressPhoneScreen extends StatefulWidget {
  ...
  @override
  State<AddUpdateAddressPhoneScreen> createState() =>
      _AddUpdateAddressPhoneScreenState();
}

class _AddUpdateAddressPhoneScreenState
    extends State<AddUpdateAddressPhoneScreen> {
  ...
  void initState() {
    ...
    if (govName != null && govName!.isNotEmpty && place != null) {
      try {
        int? buildNum = int.tryParse((place?.street ?? '').split(' ').first);
        if (buildNum != null) {
